...
IPv6 is disabled by default, to enable IPv6 modify your SecureVu configuration as follows:
...
You can change the ports Nginx uses for listening using SecureVu's configuration file. The internal port (unauthenticated) and external port (authenticated) can be changed independently. You can also specify an IP address using the format `ip:port` if you wish to bind the port to a specific interface. This may be useful for example to prevent exposing the internal port outside the container.
...
services:
  securevu:
    image: blakeblackshear/securevu:latest
    environment:
...
SecureVu currently includes go2rtc v1.9.13, there may be certain cases where you want to run a different version of go2rtc.
